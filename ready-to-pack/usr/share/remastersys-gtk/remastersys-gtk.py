...
#Modified by: Tony Brijeski <tb6517@yahoo.com>, Wallison Alves <wallisonalvesdeoliveira@gmail.com>
...
class appgui:
    def __init__(self):
        ...
        self.gladefile = "/usr/share/remastersys-gtk/remastersys-gtk.glade"
    ...
    def on_button1_clicked(self,widget):
        self.update_conf()
        if not msg_confirm(_("You have selected Backup Mode. Do not interrupt this process. Click OK to Start the Backup LiveCD/DVD/USB process."), self.window1.get_widget("window1")):
            return
    ...
    def on_backup_done(self, widget, data = None):
        if self.v.get_child_exit_status() == 0:
            ...
            msg_info(_("Your %(iso)s and %(iso)s.md5 files are ready in %(dir)s. It is recommended to test it in a virtual machine or on a rewritable cd/dvd/usb to ensure it works as desired. Click on OK to return to the main menu.") 
    ...
    def on_button2_clicked(self,widget):
        self.update_conf()
        if not msg_confirm(_("You have selected Dist Mode. Click OK to Start the Distributable LiveCD/DVD/USB process."), self.window1.get_widget("window1")):
            return
    ...
    def on_dist_done(self, widget, data = None):
        ...
        if self.v.get_child_exit_status() == 0:
            WORKDIR = self.window1.get_widget("entry6").get_text()
            CUSTOMISO = self.window1.get_widget("entry3").get_text()
            msg_info(_("Your %(iso)s and %(iso)s.md5 files are ready in %(dir)s. It is recommended to test it in a virtual machine or on a rewritable cd/dvd/usb to ensure it works as desired. Click on OK to return to the main menu.") % ({"iso" : CUSTOMISO, "dir" : WORKDIR+'/remastersys'}))
        else:
            msg_error(_("The process was interrupted!"), self.window1.get_widget("window1"))
        self.window1.get_widget("notebook1").set_current_page(0)
        self.v.handler_disconnect(self.callback_id)
    ...
    def on_button3_clicked(self,widget):
        self.update_conf()
        if not msg_confirm(_("You have selected Dist CDFS Mode. Click OK to Start the Distributable LiveCD/DVD/USB filesystem build process.")):
            return
    ...
    def on_button6_clicked(self,widget):
        ...
        about.set_authors([_("Krasimir S. Stefanov <lokiisyourmaster@gmail.com>"),_("Tony Brijeski <tb6517@yahoo.com>"),_("Wallison Alves <wallisonalvesdeoliveira@gmail.com>")])
        about.set_website("http://wikilivre.blogspot.com/p/remastersys.html")
        translators = [
            _("Bulgarian - Krasimir S. Stefanov <lokiisyourmaster@gmail.com>"),
            _("English - Krasimir S. Stefanov <lokiisyourmaster@gmail.com>"), _("Portuguese - Wallison Alves <wallisonalvesdeoliveira@gmail.com>"), _("Traditional Chinese - Kent Chang <kentxchang@gmail.com>"), _('Simplified Chinese - Mutse Young <yyhoo2.young@gmail.com>')
        ]

        about.set_translator_credits('\n'.join(translators))
        about.set_logo_icon_name('remastersys-gtk')
        license = _('''PyGTK GUI for Remastersys
Copyright (C) 2011,2016 Krasimir S. Stefanov, Tony Brijeski, Wallison Alves
Licence: GPLv3.0
http://www.gnu.org/licenses/.''')
    ...
    def load_settings(self):
        ...
        self.window1.get_widget("entry3").set_text(
            self.getvalue('CUSTOMISO', config_txt, 'custom.iso'))

        self.window1.get_widget("entry4").set_text(
            self.getvalue('EXCLUDES', config_txt, ''))
    
        self.window1.get_widget("entry5").set_text(
            self.getvalue('LIVECDURL', config_txt, 'http://wikilivre.blogspot.com/p/remastersys.html'))

        self.window1.get_widget("entry7").set_text(
            self.getvalue('SQUASHFSOPTS', config_txt, '-no-recovery -always-use-fragments -b 1M'))
...
# This is the temporary working directory and won't be included on the cd/dvd/usb
WORKDIR="%(WORKDIR)s"
...
# Here you can change the livecd/dvd/usb username
LIVEUSER="%(LIVEUSER)s"


# Here you can change the name of the livecd/dvd/usb label
...
if os.popen('whoami').read().strip() != 'root':
    ...
    if os.system('which gksu')==0:
        if os.path.exists('/usr/share/applications/remastersys-gtk.desktop'):
            os.system('gksu -D "%s" python ./remastersys-gtk.py' % _('Remastersys'))
        else:
            os.system('gksu -D "%s" python ./remastersys-gtk.py' % '/usr/share/applications/remastersys-gtk.desktop')
        ...
        if password:
            os.popen('sudo -S python ./remastersys-gtk.py','w').write(password)
else:
